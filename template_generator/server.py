...
def create_wheel_template_from_ui_inputs(inputs):
    ...
    req_coverage_area, canvas_res_str\
    = inputs
...
def make_ui_output_msg(err=None, success=None):
    ...
    ok_output = gr.Textbox.update(value=success, visible=True) if success else gr.Textbox.update(value="", visible=False)
...
def init_gradio_ui():
    ...
    with gr.Blocks(css=CSS) as g_ui:
        with gr.Row():
            ...
            with gr.Column():
                ...
                with gr.Row():
                    ...
                    real_coverage_area =    gr.Slider(0, 100, value=0, step=0.1, interactive=False, label='Actual coverage area [%]')
        ...
        output_err_textbox = gr.Textbox(show_label=False, visible=False, interactive=False, elem_classes="error-textbox")
        output_ok_textbox = gr.Textbox(show_label=False, visible=False, interactive=False, elem_classes="success-textbox")
